...
int main() {
	...
	auto query_type = [&](int t, int i, int j) {
		...
		if (j >= table.size()) {
			...
			return query_type_fast(t, i, table.size() - 1) + query_type_fast(t, 0, j - table.size());
		}
	...
	for (int a_start = 0; a_start < table.size(); a_start++) {
		...
		for (int b_t = 1; b_t <= 2; b_t++) {
			...
			int a_end = a_start + t_cnt[a_t], b_start = a_end % table.size(), b_end = b_start + t_cnt[b_t],
			    c_start = b_end % table.size(), c_end = c_start + t_cnt[c_t];

			int a_in_b = query_type(a_t, b_start, b_end), a_in_c = query_type(a_t, c_start, c_end),
			    c_in_a = query_type(c_t, a_start, a_end), c_in_b = query_type(c_t, b_start, b_end);
