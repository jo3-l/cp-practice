...
bool can_move_to(int i, int j) {
	return 0 <= i && i < N && 0 <= j && j < M && factory[i][j] != DEAD && factory[i][j] != WALL;
}
		...
		for (int j = 0; j < M; j++) {
			if (factory[i][j] == CAMERA) {
				...
				for (int k = j + 1; k < M && factory[i][k] != WALL; k++) {
					if (factory[i][k] == INITIAL)
						goto done;
					else if (factory[i][k] == EMPTY)
						factory[i][k] = DEAD;
				}
				...
				for (int z = i + 1; z < N && factory[z][j] != WALL; z++) {
					if (factory[z][j] == INITIAL)
						goto done;
					else if (factory[z][j] == EMPTY)
						factory[z][j] = DEAD;
				}
	...
	while (!q.empty()) {
		...
		while (sz--) {
			...
			for (int d = 0; d < 4; d++) {
				...
				if (can_move_to(ni, nj) && is_conveyor(factory[ni][nj]))
					tie(ni, nj) = conveyor_destination[ni][nj];
